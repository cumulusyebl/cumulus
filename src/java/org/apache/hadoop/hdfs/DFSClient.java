...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  public OutputStream create(String src,
          ...
          int buffersize)
  	throws IOException {
	  return create(src, fileSize, permission, flag, true, progress, buffersize);
}
  /**
  ...
  public OutputStream create(String src, long fileSize,
  		FsPermission permission, EnumSet<CreateFlag> flag, boolean createParent,
  	...
  	CodingMatrix matrix = null;
    try {
    	matrix = namenode.create(
          src, fileSize, writePacketSize, masked, clientName, new EnumSetWritable<CreateFlag>(flag), createParent);
    } catch(RemoteException re) {
      throw re.unwrapRemoteException(AccessControlException.class,
  	...
  	OutputStream result = new DFSOutputStream(this, src, fileSize, matrix, masked,
  			flag, createParent, progress, buffersize,
  	        conf.getInt(DFSConfigKeys.DFS_BYTES_PER_CHECKSUM_KEY, 
                    DFSConfigKeys.DFS_BYTES_PER_CHECKSUM_DEFAULT));
