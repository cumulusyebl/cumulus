  ...
  private class Packet {
    ...
    void mult(byte element) {
    	...
    	long start = System.currentTimeMillis();
    	  short[] Buf;
    	  short key;
    	  int len = this.dataPos - this.dataStart;
    	  Buf= new short[len];
    	  for (int i = 0; i < len; i++){	
    			if (this.buf[this.dataStart + i] < 0)
    				Buf[i] =(short)(this.buf[this.dataStart + i]+256);
    			else 
    				Buf[i]=(short)this.buf[this.dataStart + i];
    	
    			if(element < 0)
    				key= (short)(element + 256);
    			else key=(short)(element);
    	     Buf[i] = RSCoderProtocol.mult[Buf[i]][key];
    	  }
    	  for (int i = 0; i < len; ++i)
    	  {
...
//    	  	DFSClient.LOG.info("mult after update checksumPos: "+checksumPos); 
     ...
     * @param p
     * @return
     * precondition: this.buf.lenght >= p.buf.lenght.
     */
    ...
    void code(Packet p,byte element) {
     ...
     long start = System.currentTimeMillis();
  	  ...
  	  InputBytes= new short[len];
  	  Buf= new short[len];
  	  for (int i = 0; i < len; i++){
  			if (p.buf[p.dataStart + i]< 0)
  				InputBytes[i] =(short)(p.buf[p.dataStart + i]+256);
  			else 
  	     ...
  	     Buf[i] ^= RSCoderProtocol.mult[InputBytes[i]][key];
  	  }
  	  for (int i = 0; i < len; ++i)
  	  {
  		  this.buf[this.dataStart + i]=(byte)Buf[i];
...
//  	DFSClient.LOG.info("after update checksumPos: "+checksumPos); 
  ...
  class DataStreamer extends Daemon {
    ...
    private volatile DatanodeInfo[] nodes = null; // list of targets for current file
	     ...
	     while (!responderClosed && dfsClient.clientRunning && !isLastPacketInBlock) {
	    	 for (int i = 0; i < blockReplyStream.length; i++) {
	          ...
	          try {
	            ...
	            for (int j = ack.getNumOfReplies()-1; j >=0  && dfsClient.clientRunning; j--) {
	            ...
	            //DFSClient.LOG.info("block at index " + i + " SetNumbytes " +one.getLastByteOffsetBlock());
	            synchronized (ackQueue.get(i)) {
...
//            lastAckedSeqno = endOfBlockPacket.seqno;
    ...
    /**
     * Not suported recently. the port is reserved.
...
//       block.setNumBytes(0);
        ...
        for (int i =0; i < blocks.length; ++i) {
        	//DFSClient.LOG.info("Block at index of " + i +" :" + blocks[i].toString());
        }
...
//    }
     ...
     * @return
     */
    private boolean createBlockOutputStream(LinkedList<DatanodeInfo> datanodes, long newGS,
            boolean recoveryFlag) {
          DataTransferProtocol.Status pipelineStatus = SUCCESS;
          String firstBadLink = "";
          ...
          for(int i = 0; i < nodes.length; ++i) {
              try {
                  ...
                  InetSocketAddress target = NetUtils.createSocketAddr(nodes[i].getName());
                  ...
                  blockStream[i] = new DataOutputStream(new BufferedOutputStream(
                      ...
                      DataNode.SMALL_BUFFER_SIZE));
                  blockReplyStream[i] = new DataInputStream(NetUtils.getInputStream(s[i]));

                  // send the request
                  dn[0] = nodes[i];
                  DataTransferProtocol.Sender.opWriteBlock(blockStream[i], blocks[i], 1,
                      recoveryFlag ? stage.getRecoveryStage() : stage, newGS, 
                      blocks[i].getNumBytes(), bytesSent, dfsClient.clientName, null, dn,
                      accessToken);
                  checksum.writeHeader(blockStream[i]);
                  blockStream[i].flush();

                  // receive ack for connect
                  pipelineStatus = DataTransferProtocol.Status.read(blockReplyStream[i]);
    ...
    private LocatedBlock[] locateFollowingBlock(long start,
      ...
      while (true) {
        ...
        while (true) {
          try {
            return dfsClient.namenode.addBlock(src, dfsClient.clientName, null, excludedNodes);
...
//	    this.blockSize = blockSize;
  ...
  DFSOutputStream(DFSClient dfsClient, String src, long size, CodingMatrix matrix, FsPermission masked, 
		  EnumSet<CreateFlag> flag, boolean createParent, Progressable progress,
	      int buffersize, int bytesPerChecksum) 
	      throws IOException {
  ...
  private void computePacketChunkSize(int psize, int csize) {
    ...
    int max = (dfsClient.writePacketSize - PacketHeader.PKT_HEADER_LEN + chunkSize - 1) / chunkSize;
    if (psize > max * csize) {
    	chunksPerPacket = max;
    }
    else {
    ...
    /*DFSClient.LOG.info("computePacketChunkSize: pszie=" + psize +
            ", chunksPerPacket=" + chunksPerPacket+
             "chunkSize="+chunkSize + " //////"+checksum.getChecksumSize()+"...."+csize);
    */
    if (DFSClient.LOG.isDebugEnabled()) {
	    	...
	    	if (currentPacket.lastPacketInBlock == true) {
	    		...
	    		for (int i = 0; i < codeBuf.length; ++i) {
	    			...
	    			if (codeBuf[i] == null) {
		    			...
		    			currentPacket.setOffset(offsetInB);
		    			synchronized (dataQueue.get(i)) {
...
//		    					", checksumlen :"+ (currentPacket.checksumPos -currentPacket.checksumStart)  +
//		    					", dataPos :"+currentPacket.dataPos +
//		    					", dataStart :"+currentPacket.dataStart+
//		    					", checksumlen :"+ (currentPacket.dataPos -currentPacket.dataStart)+
...
//		    					", checksumlen :"+ (codeBuf[i].checksumPos -codeBuf[i].checksumStart)  +
...
//		    					", sum :"+ (codeBuf[i].dataPos -codeBuf[i].checksumStart));
	    				...
	    				currentPacket.setOffset(offsetInB + codeBuf[i].dataPos - codeBuf[i].dataStart);
	    				/*DFSClient.LOG.info("i: "+i+"  dataPos: "+codeBuf[i].dataPos+" dataStart:  "+codeBuf[i].dataStart
	    						 + " seqno: " + currentPacket.seqno + " BytescurrentBlock: " + bytesCurBlock);
		    			*/
		    			synchronized (dataQueue.get(i)) {
		    				dataQueue.get(i).add(currentPacket);
		    				dataQueue.get(i).notify();
  ...
  protected synchronized void writeChunk(byte[] b, int offset, int len, byte[] checksum) 
    ...
    if (currentPacket == null || currentPacket.numChunks == 0) {
    	if (appendChunk && bytesCurBlock%bytesPerChecksum == 0) {
    		...
    		if (!appendChunk) {
    			int psize = Math.min((int)(fileSize-bytesCurBlock), dfsClient.writePacketSize);
    			computePacketChunkSize(psize, bytesPerChecksum);
    			//DFSClient.LOG.info("chunksPerPacket=" + chunksPerPacket +" psize=" +psize);
    		}
    }
...
//        waitAndQueueCurrentPacket();
  ...
  private void completeFile(String src) throws IOException {
    ...
    for (Block b : blocks) {
    	//DFSClient.LOG.info("Size of " + b.getBlockId() + " = " + b.getNumBytes());
    }
    
    while (!fileComplete) {
      fileComplete = dfsClient.namenode.complete(src, dfsClient.clientName, blocks);
      if (!fileComplete) {
