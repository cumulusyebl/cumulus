...
public class FSNamesystem implements FSConstants, FSNamesystemMBean, FSClusterStats,
  ...
  public List<DatanodeDescriptor> list=new ArrayList<DatanodeDescriptor>();   // tongxin
...
//      long blockSize = trgInode.getPreferredBlockSize();
   ...
   * @param fileSize
   */
  CodingMatrix startFile(String src, long fileSize, long packetSize, PermissionStatus permissions, String holder,
		  String clientMachine, EnumSet<CreateFlag> flag, boolean createParent)
  		throws AccessControlException, SafeModeException, FileAlreadyExistsException,
  		UnresolvedLinkException, FileNotFoundException, ParentNotDirectoryException, IOException {
	  CodingMatrix matrix = startFileInternal(src, fileSize, packetSize, permissions, holder, clientMachine, flag, createParent);
	  getEditLog().logSync();
	  if (auditLog.isInfoEnabled() && isExternalInvocation()) {
		final HdfsFileStatus stat = dir.getFileInfo(src, false);
	    logAuditEvent(UserGroupInformation.getCurrentUser(),
  ...
  private CodingMatrix startFileInternal(String src, long fileSize, long packetSize,
	      ...
	      EnumSet<CreateFlag> flag, boolean createParent)
  		  throws SafeModeException, FileAlreadyExistsException,
	      AccessControlException, UnresolvedLinkException, FileNotFoundException,
	      ParentNotDirectoryException, IOException {
	    writeLock();
	    try {
...
//	      }
	      if (append) {
	        if (myFile == null) {
	          if(!create)
	            ...
	            //append & create a nonexist file equals to overwrite
	            return startFileInternal(src, fileSize, packetSize, permissions, holder, clientMachine,
	      ...
	      if (append) {
	       ...
	       checkFsObjectLimit();
	        ...
	        // increment global generation stamp
	        long genstamp = nextGenerationStamp();
	        INodeFileUnderConstruction newNode = dir.addFile(src, fileSize, packetSize, permissions,
	            holder, clientMachine, clientNode, genstamp);
	        if (newNode == null) {
	          throw new IOException("DIR* NameSystem.startFile: " +
	                                "Unable to add file to namespace.");
	        }
  ...
  public LocatedBlock[] getAdditionalBlock(String src,
    ...
    try {
      ...
      DatanodeDescriptor[] dn = new DatanodeDescriptor[1];
      for(int i = 0; i < numOfBlk; ++i) {
    	  dn[0] = targets[i];
    	  newBlock = allocateBlock(src, pathINodes, dn.clone());
    	  dn[0].incBlocksScheduled();
    	  LocatedBlock b = new LocatedBlock(newBlock, dn.clone(), 0/*offset is no need*/);
...
//      }      
   ...
   */
  public boolean completeFile(String src, String holder, Block[] blks) 
    throws SafeModeException, UnresolvedLinkException, IOException {
    boolean success = completeFileInternal(src, holder, blks);
    getEditLog().logSync();
    return success ;
  }

  private boolean completeFileInternal(String src,String holder, Block[] blks) 
  	 throws SafeModeException, UnresolvedLinkException, IOException {
    writeLock();
    try {
    if(NameNode.stateChangeLog.isDebugEnabled()) {
    ...
    // commit the last block and complete it if it has minimum replicas
    FSNamesystem.LOG.info("will call commit or complete block function.");
    blockManager.commitOrCompleteBlock(pendingFile, blks);
  ...
  /*
   * added by czl @ 2013/3/11 
   * to get all blocks' locations belong to file src
   */
  public LocatedBlocks getAllBlocksLocationsInternal(String src) throws IOException {
		for (int attempt = 0; attempt < 2; attempt++) {
			if (attempt == 0) { // first attempt is with readlock
				readLock();
			} else { // second attempt is with write lock
			...
			try {
				...
				if (isAccessTimeSupported()) {
					if (now <= inode.getAccessTime() + getAccessTimePrecision()) {
						// if we have to set access time but we only have the
						// readlock, then
						// restart this entire operation with the writeLock.
						if (attempt == 0) {
							continue;
				...
				for (int i = 0; i < blocks.length; i++) {
					results.add(blockManager.getBlockLocation(blocks[i], -1));
				}
				LOG.info("get All locations "+results.toString());
				return new LocatedBlocks(Long.MAX_VALUE, true, results, null, true);
			} finally {
				if (attempt == 0) {
					readUnlock();
				} else {
  ...
  /*
   * added by czl @ 2013/3/12
   */
 public long getFileLengthInternal(String src) throws UnresolvedLinkException {
	// TODO Auto-generated method stub
	INodeFile file = dir.getFileINode(src);
	return file.getFileSize();
 }

 public CodingMatrix getCodingMatrix(String src) throws UnresolvedLinkException {
	// TODO Auto-generated method stub
	INodeFile file = dir.getFileINode(src);
	return file.getMatrix(); 
 }
}
