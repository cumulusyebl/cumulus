...
//seq RCR_NN_CHOOSEnc.2 5
  ...
  private void commitBlock(INodeFileUnderConstruction fileINode,
    ...
    FSNamesystem.LOG.info("commitBlock Size: " + commitBlock.getNumBytes() +
    ...
    long diff = commitBlock.getNumBytes();
      try {
        String path = namesystem.leaseManager.findPath(fileINode);
  ...
  void commitOrCompleteBlock(INodeFileUnderConstruction fileINode, Block[] blks) 
  		throws IOException {
	    int n = fileINode.numBlocks();
	    if (n == 0)
	    	return; // no blocks in file.
	    for (int i = n-1; i >= 0; --i) {
	    	BlockInfo bi = fileINode.getBlock(i);
	    	if (bi.getClass() == BlockInfoUnderConstruction.class) {
		    	commitBlock(fileINode, (BlockInfoUnderConstruction)bi, blks[i]);
			   if(countNodes(bi).liveReplicas() >= minReplication)
			      completeBlock(fileINode, bi);
	    	}
